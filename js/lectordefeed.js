...
function rssdisplayer(divid, url, feedlimit, showoptions){
	...
	document.write('<div id="'+divid+'">Cargando alimentadores...</div>');
...
rssdisplayer.prototype.formatoutput=function(result){
	if (!result.error){ //if RSS feed successfully fetched
		...
		console.log("Error al obtener el alimentador: "+result.error.message);
		var caja=document.getElementById("blog");
		caja.innerHTML="";
